...
- 동기화는 HLS playlist identity·sequence·discontinuity와 timezone이 명시된 `PROGRAM-DATE-TIME`을 플랫폼 시간축으로 사용합니다. HTTP `Date`와 검증되지 않은 vendor timestamp는 재생 시간축으로 사용하지 않으며, 안전한 시간축이 없으면 각 플레이어의 라이브 엣지 거리를 기준으로 맞춥니다.
- 플레이어의 buffered/seekable 구간, `requestVideoFrameCallback` 표본, 진행 상태와 명령 결과를 확인합니다. 자동 보정은 재생 속도를 `0.98~1.02` 범위에서 조절하며 자동으로 큰 위치 이동을 실행하지 않습니다.
- 방송별로 0.1초씩 앞당기거나 늦춰 송출 장비·인코더 차이를 수동 보정할 수 있습니다. 실행 중 수동 보정값을 확정하면 seekable 범위 안에서 즉시 해당 위치로 이동합니다. 수동 보정 범위는 ±60초이며 프리셋에 저장됩니다.
- 안전 딜레이는 버퍼 상태에 따라 자동 조절됩니다. 사용자는 팝업에서 자동 조절이 내려가지 않을 최솟값을 1.5~15초로 지정할 수 있습니다.
- 정상 재생 중 작은 오차는 무시하고, 그보다 큰 오차는 제한된 배속으로 서서히 보정합니다.
...
- 이 기능은 HLS 패키징 시간축과 브라우저 플레이어 위치를 정렬합니다. 외부 공통 timecode나 영상·음성 내용 비교 없이 방송별 촬영·캡처·인코더 지연을 측정할 수 없으므로 동일 장면의 완전한 동기화나 `±N ms` 정확도는 보장하지 않습니다.
- 별도 동기화 진단 수집이나 로컬 지연 학습 기능은 포함하지 않습니다.
