...
`handoff` saves `phase0-preflight.txt`, `phase0-checklist.txt`, `phase0-audit.txt`, `phase0-verification.txt`, and a normalized `phase0-results.json` evidence snapshot together. Use `--data-folder <path>` and `--profile-folder <path>` when inspecting non-default runtime data.
...
- The CLI `handoff` command saves the preflight, checklist, audit, verification, and normalized feasibility-results JSON artifacts into one folder for manual SOOP test handoff or post-run review.
