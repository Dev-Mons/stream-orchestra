...
Use `partial` when the requested playback count or isolated group visibly plays but success-only evidence is incomplete; use `failure` when the requested playback count or group does not work. `success` is reserved for the 9+ Phase 0 success path. A `partial` record without `--account` is treated as playback evidence only and does not invalidate older same-account coverage; a matching `failure` record can invalidate older coverage for the affected profile group.
