...
`handoff` saves `phase0-preflight.txt`, `phase0-checklist.txt`, `phase0-audit.txt`, `phase0-verification.txt`, a normalized `phase0-results.json` evidence snapshot, and `phase0-handoff-manifest.json` together. Use `--data-folder <path>` and `--profile-folder <path>` when inspecting non-default runtime data.
...
- The CLI `handoff` command saves the preflight, checklist, audit, verification, normalized feasibility-results JSON, and manifest artifacts into one folder for manual SOOP test handoff or post-run review.
