...
Validate a saved handoff bundle before sharing or reviewing it:
...
`success` requires the matching 9/12/16 plan playback scenario plus `--account`, required `--profile-groups`, `--restart`, `--resources`, `--cpu-percent`, `--gpu-percent`, and `--memory-mb`; otherwise record `partial` or `failure`. Use the same non-sensitive `--account-label <text>` for every SOOP account evidence record across A-D; account labels and restart evidence are accepted only with `--account`, `--account` requires at least one `--profile-groups` value plus a label, final same-account verification requires one shared non-empty label, and conflicting labels fail the same-account gate. A 9-slot success proves the threshold scenario for A/B/C, but final verification still requires A-D same-account evidence. `--resources` always requires the CPU, GPU, and memory values so resource acceptability has structured evidence, and `failure` records cannot include `--restart` or `--resources`; normalized result loading/saving also clears those flags from hand-edited failure records. If `--scenario` is omitted, the CLI derives the scenario from `--count` using the same 4/8/9/12/16 playback-test names as the WPF app. Use `--group A`, `--group B`, `--group C`, or `--group D` for isolated profile-group evidence; without `--count`, it records the group's 4-slot test. Profile-group evidence and playback count must match the selected scenario, so Group A evidence cannot be recorded against a Group D scenario and a single-group scenario cannot be recorded as a 16-slot result; manual/custom scenarios can be recorded as `partial` or `failure`, but not `success`.

Use `partial` when the requested playback count or isolated group visibly plays but success-only evidence is incomplete; use `failure` when the requested playback count or group does not work. `failure` records reject restart and resource OK evidence; use `partial` when criteria evidence is positive but the full success path is incomplete. `success` is reserved for the 9+ Phase 0 success path. A playback-only `partial` record does not invalidate older same-account, restart, resource, or structured-observation evidence; a matching `failure` record fails the affected gate even if hand-edited criteria flags are true.
...
- Recorded feasibility results include playback count, test scenario, same-account session persistence, account label, verified profile groups, restart session persistence, structured CPU/GPU/memory observations, resource acceptability, and a decision snapshot matching the Phase 0 decision path at record time. Full plan verification requires A-D same-account profile-group evidence with one shared non-empty account label; account labels and restart evidence are saved only on same-account evidence, the WPF account-label, A-D evidence, and restart controls are enabled only when `계정 유지` is checked and are cleared when it is unchecked, `계정 유지`/`--account` cannot be recorded without at least one verified profile group and an account label, `failure` records cannot include restart or resource OK evidence, normalized result loading/saving canonicalizes known outcome casing, drops invalid or scenario-mismatched profile groups, hand-edited account/restart flags without complete same-account evidence, hand-edited restart/resource flags on failure records, resource flags without valid CPU/GPU/memory observations, `success` outcomes without complete success evidence or a matching 9/12/16 plan playback scenario, and stale decision snapshots when decision-driving evidence changes during normalization; scenario/profile-group and scenario/playback-count consistency checks prevent mismatched evidence from being saved or counted, failure evidence can invalidate older same-account coverage for the affected group, partial playback evidence without account evidence does not invalidate older coverage, and `리소스 OK` cannot be recorded without all three manual resource values. After recording, the WPF app clears the per-run CPU/GPU/memory fields and unchecks `리소스 OK` so the next playback attempt needs fresh resource observations.
...
- The WPF feasibility row shows the current playback scenario and slot count before a result is recorded, and its tooltip explains whether the scenario can satisfy a plan playback gate or is only useful as manual/account evidence.
