...
- 최상단의 **N화면 · 레이아웃 이름** 버튼을 누르면 모든 레이아웃이 화면 수별로 표시됩니다. 원하는 카드를 한 번 선택해 10화면에서 1화면처럼 화면 수를 건너뛰어 바로 전환할 수 있습니다.
...
- **Alt** — 전체 레이아웃 선택기를 열거나 닫습니다. 선택기는 키를 놓아도 유지되며, 원하는 카드를 고르거나 ESC를 누르면 닫힙니다. 화면을 줄일 때는 현재 선택한 방송을 우선 유지하고 나머지 방송을 종료합니다.
