...
Use `partial` when the requested playback count or isolated group visibly plays but success-only evidence is incomplete; use `failure` when the requested playback count or group does not work. `success` is reserved for the 9+ Phase 0 success path. A playback-only `partial` record does not invalidate older same-account, restart, resource, or structured-observation evidence; a matching `failure` record fails the affected gate even if hand-edited criteria flags are true.
