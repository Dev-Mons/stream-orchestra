...
In the WPF feasibility row, the account-label, A-D evidence, and restart controls are enabled only while `계정 유지` is checked, so playback-only partial records cannot accidentally keep stale account or restart evidence. Normalized result loading/saving also drops hand-edited account and restart flags unless same-account evidence has a label and at least one verified profile group.
