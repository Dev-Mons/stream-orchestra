...
{
    private const uint SyncPopupSetWindowPosNoSize = 0x0001;
    private const uint SyncPopupSetWindowPosNoMove = 0x0002;
    private const uint SyncPopupSetWindowPosNoActivate = 0x0010;
    private static readonly IntPtr SyncPopupHwndTopmost = new(-1);
    ...
    private void InitializeSyncFeature()
    {
        ...
        SyncPopup.Closed += (_, _) => SetSyncBadgePresentationEnabled(false);
        Deactivated += (_, _) => SyncPopup.IsOpen = false;
    ...
    private void SetSyncBadgePresentationEnabled(bool isEnabled)
    {
        foreach (var slot in _slots)
        {
            slot.SetSyncBadgePresentationEnabled(isEnabled);
        }
    }
    ...
    {
        var shouldOpen = !SyncPopup.IsOpen;
        if (shouldOpen)
        {
            // Open the slot badges first so the control popup is created last and stays above them.
            SetSyncBadgePresentationEnabled(true);
        }

        SyncPopup.IsOpen = shouldOpen;
        if (shouldOpen)
        {
            RefreshSyncUi(rebuildLists: true);
        }
    ...
    private void RefreshSyncUi(bool rebuildLists)
    {
        ...
        QueueSyncControlPopupToFront();
    }

    private void QueueSyncControlPopupToFront()
    {
        if (!SyncPopup.IsOpen)
        {
            return;
        ...
        Dispatcher.BeginInvoke(
            (Action)(() =>
            {
                ...
                SetSyncControlPopupWindowPos(
                    source.Handle,
                    SyncPopupHwndTopmost,
                    0,
                    0,
                    0,
                    0,
                    SyncPopupSetWindowPosNoMove |
                    SyncPopupSetWindowPosNoSize |
                    SyncPopupSetWindowPosNoActivate);
            }),
            DispatcherPriority.ApplicationIdle);
    }

    [DllImport("user32.dll", EntryPoint = "SetWindowPos", SetLastError = true)]
    private static extern bool SetSyncControlPopupWindowPos(
        IntPtr hWnd,
    ...
    private void RebuildSyncMemberRows(SyncGroupViewState state)
    {
        ...
        foreach (var member in state.Members)
        {
            ...
            var delayInput = new TextBox
            {
                ...
                ToolTip = "추가 지연(초), -60~60 · 적용하면 즉시 위치 이동",
